...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 22 14:18:09 2021
...
class VaiDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        """
        PointCloudDataset.__init__(self, 'Vaihingen')
        ...
        self.cloud_names = ['Vaihingen3D_Train', 'Vaihingen3D_Train']
        self.all_splits = [0, 1]
        ...
        for i, f in enumerate(self.cloud_names):
            if self.set == 'training':
                if self.all_splits[i] != self.validation_split:
                    self.files += [join(ply_path, f + '.pts')]
            elif self.set in ['validation', 'test', 'ERF']:
                if self.all_splits[i] == self.validation_split:
                    self.files += [join(ply_path, f + '.pts')]
            else:
        ...
        for i, tree in enumerate(self.input_trees):
            ...
            anchor = get_anchors(points, config.sub_radius, xyz_offset=config.xyz_offset, method=config.anchor_method)
            anchor, anchor_tree, anchors_dict, achor_lb = anchors_part_lbs(tree, anchor, self.input_labels[i], config.sub_radius, config.num_classes)
            # update sub_region information according to averlaps
            anchor, anchor_tree, anchors_dict, achor_lb = update_anchors(tree, anchor, anchor_tree, anchors_dict, achor_lb, config.sub_radius)
            print('number of anchors: ', len(anchors_dict.keys()))# cls_lbs = part_class_lbs(anchors_dict, self.input_labels[i])
        ...
        if use_potentials:
            ...
            for i, tree in enumerate(self.pot_trees):
                self.potentials += [torch.from_numpy(np.random.rand(tree.data.shape[0]) * 1e-3)] #Create an array of the given shape and populate it with random samples from a uniform distribution over [0, 1).
            ...
            # Share potential memory convert list to numpy to tensor
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            with self.worker_lock:
                ...
                cloud_ind = int(torch.argmin(self.min_potentials)) # find which cloud has lower potential
                point_ind = int(self.argmin_potentials[cloud_ind]) # in that cloud, find which point has the lowest potential from the coarsened point cloud

                # Get potential points from tree structure
                pot_points = np.array(self.pot_trees[cloud_ind].data, copy=False) # get the coarsened cloud
                ...
                # Indices of points in input region  # query a spherical region centered on the center_point, radius = in_radius # pot_inds are neighboring points idx, dis are distances to the center_point
                ...
                if self.set != 'ERF':
                    ...
                    if self.set!='training':
                        ...
                        self.min_potentials[[cloud_ind]] = self.potentials[cloud_ind][min_ind] #find new minmum potential
                        self.argmin_potentials[[cloud_ind]] = min_ind  # find the new point with minmum potential
                    else:
                        self.potentials[cloud_ind][point_ind] += 0.01   # near to the center_point, higher weights will be added
                        min_ind = torch.argmin(self.potentials[cloud_ind])
                        self.min_potentials[[cloud_ind]] = self.potentials[cloud_ind][min_ind] #find new minmum potential
                        self.argmin_potentials[[cloud_ind]] = min_ind  # find the new point with minmum potential
            ...
            # Get points from tree structure  # subsampled points with dl0
            ...
            region_lb = []
            region_idx = []
            # map the ids in the original point clouds to the selected point clouds
            for aa in range(pot_anchor_inds[0].shape[0]):
                pot_ans_idx = pot_anchor_inds[0][aa] # get anchor id
                idx_r = pot_anchor_dict[pot_ans_idx][0][0] # get corresponding pts id
                tt_0 = time.time()
                # idx0 = np.where(idx_r.reshape(idx_r.shape[0],1)==input_inds)[1]
                x = input_inds
            ...
            input_points = (points[input_inds] - center_point).astype(np.float32) # normalize the data
            ...
            input_points, scale, R = self.augmentation_transform(input_points) # use augmentation_transform
            ...
            if batch_n > 1: # int(self.batch_limit):
                break
...
#        print('sub_time: ', time.time()-tt1)
        t += [time.time()]

        # Add scale and rotation for testing
        input_list += [scales, rots, cloud_inds, point_inds, input_inds, cloud_lb, cloud_all_lb, region_list, region_lb_list, cen_pts]
    ...
    def prepare_Vai_ply(self):
        ...
        for cloud_name in self.cloud_names:
            ...
            cloud_colors = np.empty((0, 1), dtype=np.float32)
            ...
            for i, room_folder in enumerate(room_folders):
                ...
                for object_name in listdir(join(room_folder, 'Annotations')):
                    ...
                    if object_name[-4:] == '.txt':
                        ...
                        cloud_colors = np.vstack((cloud_colors, object_data[:, 3:4].astype(np.uint8)))
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                data = np.genfromtxt(file_path, delimiter=' ', defaultfmt=fmt)
                points = data[:,:3].astype('float32')  
                colors = data[:,3].astype('float32') 
                ...
                # Get chossub_colorsen neighborhoods
...
class VaiSampler(Sampler):
    """Sampler for S3DIS"""

    def __init__(self, dataset: VaiDataset):
...
def VaiCollate(batch_data):
    return VaiCustomBatch(batch_data)
