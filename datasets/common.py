...
class PointCloudDataset(Dataset):
    ...
    def segmentation_inputs(self,
        ...
        # Starting radius of convolutions  0.24*2.5=0.6
        r_normal = self.config.first_subsampling_dl * self.config.conv_radius  
        ...
        for block_i, block in enumerate(arch):
            ...
            # Get all blocks of the layer
            if not ('pool' in block or 'strided' in block or 'global' in block or 'upsample' in block or 'attention' in block):
                layer_blocks += [block]
                continue
            ...
            if layer_blocks:
                ...
                if np.any(['deformable' in blck for blck in layer_blocks]):
                    ...
                    r = r_normal
                conv_i = batch_neighbors(stacked_points, stacked_points, stack_lengths, stack_lengths, r) #return: neighbors indices
                # for each point in stacked_points, find neighoring points in stacked_points in radius r=dl*conv_raduis
            else:
                # This layer only perform pooling, no neighbors required
                conv_i = np.zeros((0, 1), dtype=np.int32)
            ...
            if 'pool' in block or 'strided' in block:
                ...
                # Subsampled points # pool_b length # increase the dl
                pool_p, pool_b = batch_grid_subsampling(stacked_points, stack_lengths, sampleDl=dl) # return: subsampled points, with features and/or labels depending of the input
                ...
                # Subsample indices # for each point in pool_p, find neighoring points in stacked points within r radius
                pool_i = batch_neighbors(pool_p, stacked_points, pool_b, stack_lengths, r) # return: neighbors indices
