...
class Config:
    ...
    def save(self):
        ...
        with open(join(self.saving_path, 'parameters.txt'), "w") as text_file:
            ...
            if hasattr(self, 'cl_method'):
                text_file.write('cl_method = {:s}\n'.format(self.cl_method))       
            ...
            if hasattr(self, 'pretxt'):
                text_file.write('pretxt = {:s}\n'.format(self.pretxt))       
            if hasattr(self, 'use_dropout'):
                text_file.write('use_dropout = {:.3f}\n'.format(self.use_dropout))                   
    def update_arg(self, argss):
        for k in dir(argss):
            # print(k)
            if '__' in k:
                pass
                ...
                if k in dir(self):
                    setattr(self, k, getattr(argss, k))
                    # print(k)
