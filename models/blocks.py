...
def index_normalize(idx_tensor):
    # tensor shape []
    aa = idx_tensor-idx_tensor.min(dim=1, keepdim=True)[0]
    aa_max = idx_tensor.max()
    replace_mask = idx_tensor == aa_max
    replace_num = idx_tensor.shape[0]
    aa[replace_mask]=replace_num
...
class ResnetBottleneckBlock(nn.Module):
    ...
    def forward(self, features, batch):
        ...
        # print(q_pts.size(), s_pts.size(), neighb_inds.size())
        # Convolution
...
class channel_att_new2(nn.Module):
...
class dual_att(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        ...
        if self.drop:
            self.droplayer = Dropout(p=config.use_dropout)
